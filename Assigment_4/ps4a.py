...
def updateHand(hand, word):
    ...
    newHand = hand.copy()
    for letter in word:
        newHand[letter] = newHand.get(letter, 1) - 1
    return newHand
...
def isValidWord(word, hand, wordList):
    ...
    isValid = True
    if word in wordList:
        letterFrequency = getFrequencyDict(word)
        for key in letterFrequency:
            if not (key in hand and hand[key] >= letterFrequency[key]):
                isValid = False
                break
    else:
        isValid = False
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    totalScore = 0
    handCopy = hand.copy()
    lengthHand = calculateHandlen(handCopy)
    ...
    while lengthHand > 0:
        ...
        if userInput == ".":
            ...
            if not isValidWord(userInput, handCopy, wordList):
                ...
                print('"' +str(userInput) + '" earned ' + str(pointsEarned) + ' points. Total: ' + str(totalScore) + ' points')
                handCopy = updateHand(handCopy, userInput)
                lengthHand = calculateHandlen(handCopy)

    if lengthHand <= 0:
        print("Run out of letters. Total score: " + str(totalScore) + ".")
